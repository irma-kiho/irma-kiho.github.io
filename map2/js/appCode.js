...
import * as myModule from "./layers.js"
...
async function initializeLayers(){ 
  ...
  const overlayLayers = {
   ...
   loadWmsLayers(myModule.wmsLayers, overlayLayers, activeWmsLayers)
...
map.on('click', function(event) {
  ...
  Object.entries(activeWmsLayers).forEach(([key, value]) => {
    ...
    // console.log("Querying:", key) POLE VAJA?
    //  console.log("URL:", fullUrl) POLE VAJA?
    }
  })
})
 //POLE VAJA?
  ...
  fetch(fullUrl)
    ...
    .then(data => {
      ...
      if (data.features && data.features.length > 0) {
        ...
        const properName = getLayerName(myModule.wmsLayers, layerName)


        // show the title of the layer
        let html = `<h4>${properName}</h4><ul>`
        ...
        content.innerHTML += `<em>No features found for ${properName}</em><br>`
      }
